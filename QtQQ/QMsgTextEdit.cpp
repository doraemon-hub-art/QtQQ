...
void QMsgTextEdit::onEmotionImageFrameChange(int frame)
{
	...
	document()->addResource(QTextDocument::ImageResource,QUrl(m_emootionMap.value(movie)),movie->currentPixmap());
}
...
void QMsgTextEdit::addEmotionUrl(int emotionNum)
{
	...
	m_emootionMap.insert(apngMovie, flagName);
...
{
	for (auto itor = m_emootionMap.constBegin(); itor != m_emootionMap.constEnd(); itor++) {
		delete itor.key();
	}
	m_emootionMap.clear();
}
